...
class DistinctExecutor : public AbstractExecutor {
 ...
 private:
  ...
  /** The set of keys that have encountered*/
  std::unordered_set<DistinctKey> key_set;
  
  /** @return The tuple as an DistinctKey */
  auto MakeDistinctKey(Tuple *tuple) -> DistinctKey {
    std::vector<Value> vals;
    for (uint32_t i = 0; i < plan_->OutputSchema()->GetColumnCount(); i++) {
      vals.push_back(tuple->GetValue(plan_->OutputSchema(), i));
    }
    return {vals};
  }
};
